...
export function ExportButton({ data, shopName }: Props) {
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `Inventory_Report_${shopName}_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  };
  ...
  const exportToPDF = () => {
    ...
    doc.save(
      `Inventory_Report_${shopName}_${new Date().toISOString().split("T")[0]}.pdf`,
    );
  };

  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="inline-flex items-center gap-2 text-sm rounded-xl border border-gray-200 shadow-sm font-bold bg-white hover:bg-gray-50 text-blue-900 px-4 py-2 transition-colors cursor-pointer">
      ...
      <DropdownMenuContent
        align="end"
        className="rounded-xl p-1 shadow-lg border-gray-100 bg-white min-w-[160px]"
      >
        <DropdownMenuItem
